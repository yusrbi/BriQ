...
def get_mentions_and_clear(text, sent_start_offset, sent_end_offset,regex):
    ...
    for match in re.finditer(regex,text):
        ...
        if len(mention_text) <= 1:
            continue
        start_offset = sent_start_offset+match.start()
        end_offset =sent_start_offset+match.end()

        mention = Mention(start_id, mention_text, start_offset, end_offset)
        start_id = start_id +1
...
def extract_mentions_from_text(text):
    ...
    all_mentions=[]
    start_id = 1
    mentions = run_mention_extraction_in_order(text,0,len(text))
    all_mentions.extend(mentions)
    return all_mentions
