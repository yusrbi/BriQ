...
alpha = 0.3 # stationary value
beta = 0.7 # prior
threshold = 0.4 # for selecting True relation
count_global_fn =0
count_global_fn2=0
lambda1 =0.7 # weight
lambda2 = 0.3 #surfaceform
...
def get_distinct_mentions(priors):
    ...
    for prior in priors:
        ...
        if prior[-3].strip() == 'X1' and prob < threshold:
...
def generate_params_values():
    ...
    for alpha in np.linspace(0.1, 0.9, 9):
        beta = 1- alpha
        for threshold in np.linspace(0.2,0.5,4):
            for lambda1 in np.linspace(0.1,0.9,9):
                lambda2 = 1- lambda1
...
def param_tuning(edges_folder, priors_file):
    ...
    for alpha_, beta_, threshold_, lambda1_, lambda2_ in values:
        ...
        try:
            ...
            if f1 > best_accuracy:
                best_accuracy = f1 
                all_best_accuracy.append((f1,(alpha,beta,threshold,lambda1,lambda2)))
                best_params = (alpha,beta,threshold,lambda1,lambda2)
                print("So FAR Best f1: %s"%f1)
                print("So FAR Alpha\tbeta\tthreshold\t,lambda1, lambda2")
                print("%s\t%s\t%s\t%s\t%s"%best_params)
                sys.stdout.flush()
            elif f1 ==best_accuracy:
                all_best_accuracy.append((f1,(alpha,beta,threshold,lambda1,lambda2)))

        except Exception as e:
            print(e)
            traceback.print_exc()
            print("Could Not tune params")
            
    print("Best f1: %s"%best_accuracy)
    print("Best params: Alpha\tbeta\tthreshold\t,lambda1, lambda2")
    if best_params:
        print("%s\t%s\t%s\t%s\t%s"%best_params)
    for f1, params in all_best_accuracy:
        print(",f1: %s"%f1)
        print("params: %s\t%s\t%s\t%s\t%s"%params)
...
def test():
        ...
        priors =[['1','6.1','hg',1,1,0,'0.7'],['2','7.3', 'jsjs',1,0,1,'0.7'],['2','5.4','hdh','iei',1,1,1,'0.6'],['3','7.3','gsh',1,1,1,'0.2']]
...
def main():
    ...
    param_tuning(edges_folder, priors_file)
