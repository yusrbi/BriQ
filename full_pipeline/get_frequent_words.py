...
import extractor
import codecs
from sentence_processor import get_most_frequent_words
...
#       logger = logging.getLogger("py4j")
        ...
        doc_loader = Document_Loader(1700,'')
        ...
        while doc_loader.load_next_batch():
            ...
            doc_rdd.map(feature_extractor.extract_features_for_document)\
                    .flatMap(lambda x: x.get_local_relation_words())\
                    .reduceByKey(lambda a,b: a+b)\
                    .map(lambda (rel, words): (rel,get_most_frequent_words(words)))\
                    .saveAsTextFile('frequent-words_local/batch%s'%(batch_no))
