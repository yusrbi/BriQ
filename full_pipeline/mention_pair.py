...
class MentionPair:
    ...
    def process(self,text_mention, table_mention):
        ...
        #case two weak match
        #elif not text_unit and not table_unit:
        #    self.unit_sim = 0.5
        #case three weak mismatch
        else:
            self.unit_sim = 0.0
        ...
        if text_aggr and table_aggr:
            self.aggr_func_sim = utils.get_aggregate_function_sim(text_aggr, table_aggr)
        ...
        self.tokens_overlap = utils.get_weighted_tokens_overlap(text_mention.get_sentence_start_offset(), text_mention.get_context_tokens(), table_mention.get_context_tokens(), 1, 0.025)
        text_mention.update_max_ltokens_sim(self.tokens_overlap)

        self.global_tokens_overlap = utils.get_weighted_tokens_overlap(text_mention.start_offset,  text_mention.get_global_context_tokens(), table_mention.get_global_context_tokens(),5, 0.1)
    ...
    def get_as_csv_line(self):
        ...
        line = '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%d\t%s\n'%(
            ...
            self.unit_sim, self.modifier,self.aggr_func_sim,self.tokens_overlap/self.text_mention.get_max_ltokens_sim(), self.np_sim,self.global_tokens_overlap/self.text_mention.get_max_gtokens_sim(),self.global_np_sim,self.surface_form_sim,self.get_approx_sim(),self.text_mention_val, self.table_mention_val,self.text_mention.features['scale'], self.table_mention.features['scale'],self.get_ground_truth(),self.aggregate_function)
    ...
    def get_as_list(self):
        ...
        self.unit_sim, self.modifier,self.aggr_func_sim, self.tokens_overlap/self.text_mention.get_max_ltokens_sim(), self.np_sim,self.global_tokens_overlap/self.text_mention.get_max_gtokens_sim(),
