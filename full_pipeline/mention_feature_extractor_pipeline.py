...
#       logger = logging.getLogger("py4j")
        ...
        while doc_loader.load_next_batch():
            ...
            #print(doc.doc_id)
            # extract features
            #doc_rdd.flatMap(lambda x: x.get_all_edges_as_csv_line()).saveAsTextFile('results_%sneg_trunc/edges_%s'%(n_negative_instances,batch_no))
            ...
            #    .saveAsTextFile('results_%sneg_trunc/output_%s'%(n_negative_instances,batch_no))
            
            doc_rdd.map(feature_extractor.extract_features_for_document)\
                .flatMap(lambda x: x.get_mention_features_for_classification())\
                .saveAsTextFile('mention_features/batch_%s'%(batch_no))
            #doc_rdd.map(feature_extractor.extract_features_for_document)\
            #        .flatMap(lambda x: x.get_relation_all_words())\
            #        .reduceByKey(lambda a,b: a+b)
            #        .map(lambda(key, value):get_most_frequent_words )
            '''
