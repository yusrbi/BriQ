...
class PipeLine:
    def __init__(self):
        ...
        self.conf = SparkConf().setAppName("Extract Mention Pairs with Features").setMaster('local')
        self.sc = SparkContext(conf=self.conf)
    def __enter__(self):
        return self      
    def start(self, n_negative_instances, m_type):
        # spark context 
        sc = self.sc
        # load documents 
        batch_no =1 
        Document.m_type = m_type
...
#       logger = logging.getLogger("py4j")
        ...
        while doc_loader.load_next_batch():
            ...
            #print(doc.doc_id)
            # extract features
            doc_rdd.flatMap(lambda x: x.get_all_edges_as_csv_line()).saveAsTextFile('results_%sneg_%s/edges_%s'%(n_negative_instances,m_type,batch_no))
            ...
            doc_rdd.map(feature_extractor.extract_features_for_document)\
                ...
                .saveAsTextFile('results_%sneg_%s/output_%s'%(n_negative_instances, m_type,batch_no))
            
            #doc_rdd.map(feature_extractor.extract_features_for_document)\
            #flatMap(lambda x: x.get_mention_features_for_classification()).saveAsTextFile('mention_features/batch_%s'%(n_negative_instances,batch_no))
