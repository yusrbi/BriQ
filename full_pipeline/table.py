...
class Table(object):
    def __init__ (self, table_id, table_num, table_html, table_caption):
        ...
        self.scale = 1
        self.unit = None
        self.n_non_empty_cells = 0
        self.n_quantity_cells =0
        self.general_context_tokens = None
        self.consider_table = True

        self.table_to_matrix()
        #if not self.consider_table:
        #    return # do not proceed with mention extraction
    ...
    def get_table_table_edge_list(self, doc_id):
        ...
        for i in range(len(cells)):
            ...
            for j in range(i+1, len(cells)):
                ...
                proximity = 0.1
                edges.append([doc_id, m1,m2,proximity,surfaceform_sim,val_diff])
        ...
        for cells, aggr_mentions in self.aggregate_mentions.iteritems():
            ...
            ##based on the max #n of cols or rows, 20, set this to 1/10
            proximity = 0.1
            for cell in cells:
                cell = int(cell)
    ...
    def get_mentions_count(self):
        if self.mentions:
            return len(self.mentions.values())
        else:
            return 0

    def get_aggregate_mentions_count(self):
        if self.aggregate_mentions:
            return len(self.aggregate_mentions.values())
        else:
            return 0
    ...
    def compute_percent(self,key, values):
        ...
        percent = abs(round(percent,2))# the absolute value as we consider sign for computing aggregates, but we don't consider sign anywhere else

        mention = Mention(key, str(percent)+'%', 0,0)
        mention.set_table_id(self.table_id)
        mention.add_aggregate('percentage',[cell_indx1, cell_indx2],'%')
        ...
        mention.precesion = 2 #max(prec1,prec2) # the max os the 2 quantities precision
        mention.unit='%'
    ...
    def compute_diff(self,key, values):
        ...
        diff = abs(value1-value2) #"%s-%s"%(value1,value2) #value1-value2# the absolute value as we consider sign for computing aggregates, but we don't consider sign anywhere else
    ...
    def compute_ratio(self,key, values):
        ...
        ratio = abs(round(ratio,2))# the absolute value as we consider sign for computing aggregates, but we don't consider sign anywhere else

        mention = Mention(key, str(ratio)+'%',0,0)
        mention.set_table_id(self.table_id)
        mention.add_aggregate('rat',[indx1,indx2],'%')
        inverse = 100.0*abs(value2-value1)/float(value1)#"(%s-%s)/%s"%(value2,value1,value1) #(value2-value1)/value1
        inverse = abs(round(inverse,2))
        mention.add_aggregate_inverse(inverse, str(inverse)+'%')
        mention.precision = 2 #max(prec1,prec2)
        mention.unit ='%'
        self.aggregate_mentions[key].append(mention)
    ...
    def compute_sum_and_avg(self,key,  values):
        ...
        if sum_ == 0:
            return
        sum_ = abs(sum_) # the absolute value as we consider sign for computing aggregates, but we don't consider sign anywhere else
    ...
    def table_to_matrix(self):
        ...
        row_count= len(rows)
        if row_count <=1 or row_count > 100 or  col_count <= 1 or col_count > 50 :
            self.consider_table = False
        self.general_context_tokens = []
...
#                print(self.html)
        ...
        for row in rows:
            ...
            for col in columns:
                ...
                if col_content:
                    self.n_non_empty_cells = self.n_non_empty_cells +1
                    if utils.isQuantity(col_content):
                        self.n_quantity_cells = self.n_quantity_cells +1
                    #else:
                    #save tokens
                    # add all to context 
                    self.general_context_tokens.extend(utils.get_word_list(col_content))
                    ...
                    for j in range(reps):
                        ...
                        while matrix[row_indx+i][col_indx +j][0] != '':#skip non empty columns
                            ...
                            if col_indx +j >= col_count:
                                ...
                                break
                        if row_indx < row_count and col_indx < col_count:
        ...
        if self.n_non_empty_cells !=0:
            ratio_of_q_cells = float(self.n_quantity_cells) /float (self.n_non_empty_cells)
        else:
            ratio_of_q_cells = 0 
        #self.general_context_tokens = set(self.general_context_tokens)
        if ratio_of_q_cells <0.3:
            self.consider_table = False
        #print("table context: %s"%self.general_context_tokens)

    def get_header_keywords_count(self):
        return [(word, 1) for word in self.header_tokens]
    def get_keywords_count(self):
        words = []
        if self.row_context:
            words.extend( self.row_context[0])
        if self.col_context:
            words.extend(self.col_context[0])
        words = set(words)
    ...
    def extract_mentions(self):
        ...
        for i  in range(self.row_count):
            for j  in range(self.col_count):
                ...
                if indx in self.mentions:
                    ...
                    if utils.isQuantity(col_val) and not  (i==0 and utils.isYear(col_val)):
                        #print( indx )
                        mention = Mention(indx, col_val, 0, len(col_val))
                        #print mention
                        mention.set_table_id(self.table_id)
                        self.mentions[indx] = mention
                        if j ==0 or i ==0  :# in case the quantity is the first cell in the row or the column  add it to context
                            tokens, nps = extract_noun_phrases(col_val)
                            for t in tokens:
                                if t in NUMBERS:
                                    tokens.append(NUMBERS[t])
                    ...
                    # Not a Quantity add to the column and row contexti
                        #print(col_val)
                        tokens, nps = extract_noun_phrases(col_val)
                        self.row_context[i].extend(tokens)#column index is not needed
                        self.row_np[i].extend(nps)
            ...
            if len(self.row_context[i]) >0:
                ...
                if scale and scale != 1  or unit:
                    self.row_meta[i]['scale'] = scale
                    self.row_meta[i]['unit'] = unit
                    if self.scale == 1  and not self.unit:
                        #print("setscale", scale)
                        self.scale = scale
                        self.unit = unit
                elif self.scale ==1 and i ==0: #in case of table has no scale and in  the first row:
                    content = self.matrix[i][0][1]
                    scale = utils.get_zero_based_scale(content)
        ...
        for col in range(self.col_count):
            if len(self.col_context[col]) > 0:
                ...
                if scale and scale != 1  or unit:
                    #print("col scale, unit", col, scale, unit)
                    self.col_meta[col]['scale'] = scale
                    self.col_meta[col]['unit'] = unit

        #print(self.row_context)
        #print(self.col_context)
    ...
    def get_mention_context_words(self,mention):
        ...
        for cell in cells.split(','):
            #print 'cell:%s'%cell
            row,col = self.get_first_matrix_row_col(int(cell))
            context.extend(self.get_context_words_row(row))
            context.extend(self.get_context_words_col(col))
    ...
    def get_context_words_table(self):
        context = []
        context.extend(self.header_tokens)
        context.extend(self.footer_tokens)
        context.extend(self.caption_tokens)
        #add the context of column 0 and row0
        if(len(self.col_context) >0):
            context.extend(self.col_context[0])
        if(len(self.row_context)>0):
            context.extend(self.row_context[0])
        return set(context)
    def get_np_table(self):
        np = []
        np.extend(self.header_np)
        np.extend(self.footer_np)
        np.extend(self.caption_np)
        # add the np of column0 and row 0
        if(len(self.col_np) >0):
            np.extend(self.col_np[0])
        if(len(self.row_np) >0):
            np.extend(self.row_np[0])
        return set(np)
    ...
    def get_mention_noun_phrases(self,mention):
        ...
        for cell in cells.split(','):
            ...
            row,col = self.get_first_matrix_row_col(int(cell))
            nps.extend(self.row_np[row])
            nps.extend(self.col_np[col])
