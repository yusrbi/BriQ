...
class FeatureExtractor:
    ...
    def extract_mention_pairs_with_features(self, document):
        ...
        for mention in document.mentions:
            ...
            for table in document.tables:
                ...
                for table_mention in table.get_mentions():
                    ...
                    #if mention_pair.get_approx_value_difference() > 0.5:
                    #    continue # skip mention pairs that has a difference more than half the value of the max value of the two
            ...
            max_diff, max_no_scale_diff = self.get_max_differences(mention_pair_list)

            mention.set_max_relative_difference(max_diff)
            mention.set_max_no_scale_difference(max_no_scale_diff)
        ...
        document.set_mention_pairs(mention_pairs.values())
        self.logger.append("Done Extracting Mention Pairs for document: %s"%document.doc_id)
        return mention_pairs.values() #document
...
#                print(features)
        local_aggr_counts = utils.count_aggr_kw(tokens)
        global_aggr_counts = utils.count_aggr_kw(all_doc_tokens)
