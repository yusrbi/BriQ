...
test <- function(mydata, classifier, input_dir){
  ...
  # on test
  pred = predict(model, newdata=data.test,  type='prob')
  pred.labels = predict(model, newdata=data.test)
  confusion_matrix<-confusionMatrix(data =pred.labels, positive='X1', reference=data.test$GT)
  print(confusion_matrix)
  data.test$prob <- pred$X1
  #write.table(data.test, file=paste(c(input_dir,"/evaluation_results/output_plus_prob_",classifier, "_test.tsv"), collapse = ""), quote=FALSE, sep='\t', col.names = NA)
  print_results('All', confusion_matrix)
  
  
  print("######## Sum Results #########")
  confusion_matrix_sum = confusionMatrix(data =pred.labels[grep('sum',data.test$mTid)], positive='X1',reference=data.test[grep('sum',data.test$mTid),]$GT)
  print(confusion_matrix_sum)
  print_results('Sum', confusion_matrix_sum)
  print("######## difference Results #########")
  confusion_matrix_diff =  confusionMatrix(data =pred.labels[grep('dif',data.test$mTid)], positive='X1',reference=data.test[grep('dif',data.test$mTid),]$GT)
  print(confusion_matrix_diff)
  print_results('Difference', confusion_matrix_diff)
  print("######## Percent Results #########")
  confusion_matrix_perc = confusionMatrix(data =pred.labels[grep('perc',data.test$mTid)], positive='X1',reference=data.test[grep('perc',data.test$mTid),]$GT)
  print(confusion_matrix_perc)
  print_results('percentage', confusion_matrix_perc )
  print("######## Ratio Results #########")
  confusion_matrix_rat = confusionMatrix(data =pred.labels[grep('rat',data.test$mTid)], positive='X1',reference=data.test[grep('rat',data.test$mTid),]$GT)
  print(confusion_matrix_rat)
  print_results('Ratio', confusion_matrix_rat)
  print("######## Avg Results #########")
  confusion_matrix_avg =  confusionMatrix(data =pred.labels[grep('avg',data.test$mTid)], positive='X1',reference=data.test[grep('avg',data.test$mTid),]$GT)
  print(confusion_matrix_avg)
  print_results('Average', confusion_matrix_avg)
  print("######## Same Results #########")
  confusion_matrix_same = confusionMatrix(data =pred.labels[grep('^\\d+\\.\\d+$',perl=T, data.test$mTid)],
                                          positive='X1',reference=data.test[grep('^\\d+\\.\\d+$', perl = T,data.test$mTid),]$GT)
  print(confusion_matrix_same)
  print_results('Same', confusion_matrix_same)
  
  
  print("######## virtual cells Results #########")
  confusion_matrix_virtual = confusionMatrix(data =pred.labels[grep('^\\d+\\.\\d+\\,.*$',perl=T, data.test$mTid)],
                                             positive='X1',reference=data.test[grep('^\\d+\\.\\d+\\,.*$', perl = T,data.test$mTid),]$GT)
...
startTrain <- function(input_dir, test_doc_ids_file_name){
  ...
  print(all_results)
  output_file = paste0(c(input_dir, "/evaluation_results/classifiers_test_results.tsv"), collapse = "")
...
for ( i in 1:10){
  ...
  print(paste0("Processing Time: ", end_time-start_time))
}
