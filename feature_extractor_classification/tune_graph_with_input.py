...
def get_distinct_mentions(priors):
    ...
    for prior in priors:
        ...
        if prior[-6].strip() == 'X1' and prob < threshold:
...
def param_tuning(edges_folder, priors_file):
    ...
    for alpha_, beta_, threshold_, lambda1_, lambda2_ in values:
        ...
        try:
            ...
            if precision > best_accuracy:
                ...
                all_best_accuracy.append((precision,(alpha,beta,threshold,lambda1,lambda2)))
                best_params = (alpha,beta,threshold,lambda1,lambda2)
                print("So FAR Best precision: %s"%precision)
                print("So FAR Alpha\tbeta\tthreshold\t,lambda1, lambda2")
                print("%s\t%s\t%s\t%s\t%s"%best_params)
                sys.stdout.flush()
            elif precision ==best_accuracy:
                all_best_accuracy.append((precision,(alpha,beta,threshold,lambda1,lambda2)))
    ...
    print("Best precision: %s"%best_accuracy)
    print("Best params: Alpha\tbeta\tthreshold\t,lambda1, lambda2")
    if best_params:
        print("%s\t%s\t%s\t%s\t%s"%best_params)
    for precision, params in all_best_accuracy:
